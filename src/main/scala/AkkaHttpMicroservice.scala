...
case class IpTestRequest(ip1: String)
...
trait Protocols extends ErrorAccumulatingCirceSupport {
  ...
  implicit val ipTestRequestDecoder: Decoder[IpTestRequest] = deriveDecoder
  implicit val ipTestRequestEncoder: Encoder[IpTestRequest] = deriveEncoder
...
//curl -v "http://localhost:8085/features/default/cbd4172a-0d66-4702-90c2-208698b1a483/NGQYvWIEWFUrHA1e61c1pHpDlX6Tvx\*mAnxxZRr0iNOAGrjMUOr"
  def featchFeatureFlag(key: String) : Future[Boolean] = {
    ...
    val repo = config.getRepository()
    println(repo.getAllFeatures())
    val ctx = config.newContext()
    val result = ctx.feature(key).getBoolean()
